...
#define S_FUNCTION_NAME rtdacpci_abs_vmax
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    int    iVMax, iVDiv, iLimit;
    real_T *y = ssGetOutputPortRealSignal(S,0);

    if( BaseAddress == 0 ) return;
 
    iVMax = (int)(_inpd( (unsigned short)(BaseAddress + 0x5C) ) );
    iVDiv = (int)(_inpd( (unsigned short)(BaseAddress + 0x50) ) );
    iLimit = (int)(_inpd( (unsigned short)(BaseAddress + 0x58) ) ); 
    *(y+0) = iVMax;
    if ( (int)(_inpd( (unsigned short)(BaseAddress + 0xAC) ) && 0x1) != 0)
      *(y+2) = 1;
    else
      *(y+2) = 0;
}
