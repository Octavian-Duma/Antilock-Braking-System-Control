...
#define S_FUNCTION_NAME rtdacpci_abs_pulsewidth
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    HLen = _inpd( (unsigned short)(BaseAddress + 180) );
    LLen = _inpd( (unsigned short)(BaseAddress + 184) );
    if ( (LLen == 0) || (HLen == 0) )
      *(y+0) = 0;
    else
      *(y+0) = 235.0-(400.0*HLen/LLen);

    *(y+1) = _inpd( (unsigned short)(BaseAddress + 168) );
    if (_inpd( (unsigned short)(BaseAddress + 0xAC) ) && 0x4 != 0)
      *(y+2) = 1;
    else
      *(y+2) = 0;
}
